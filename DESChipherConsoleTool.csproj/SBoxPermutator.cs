...
namespace DESChipherConsoleTool
{
    public static class SBoxPermutator
    {
        ...
        /// <summary>
        /// Метод для применения перестановки S-блоками
        /// </summary>
        /// <param name="input">Битовый массив на вход</param>
        /// <returns>Сжатый блок бит</returns>
        public static BitArray Substitution(BitArray input)
        {
            BitArray output = new BitArray(32);
            for (int i = 0; i < 8; i++)
            {
                int row = (input[i * 6] ? 2 : 0) | (input[i * 6 + 5] ? 1 : 0);
                int col = (input[i * 6 + 1] ? 8 : 0) | (input[i * 6 + 2] ? 4 : 0) | (input[i * 6 + 3] ? 2 : 0) | 
                    (input[i * 6 + 4] ? 1 : 0);
